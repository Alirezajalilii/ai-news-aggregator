"""
AI News Aggregator - MarkTechPost Scraper
...
class MarkTechPostScraper(BaseScraper):
    ...
    async def parse_articles(self, soup: BeautifulSoup, url: str) -> List[ArticleData]:
        ...
        for card in article_cards[:20]:
            try:
                ...
                logger.debug(f"Error parsing MarkTechPost article: {e}")
                continue
    ...
    def _extract_article(self, card, base_url: str) -> ArticleData:
        ...
        if article_url and not article_url.startswith("http"):
            article_url = f"https://www.marktechpost.com{article_url}"
...
ScraperRegistry.register("marktechpost", MarkTechPostScraper)
